...
int		TypeConv( TextAnalyse *txt, char *CSType, char *CSTypeName, bool UseSize, char *CPPType, bool *pOutFlag, bool Struct, bool Unsafe );		// C++ 用の型文字列を C# 用の型文字列に変換する( 戻り値：型のサイズ( byte数 ) )
...
int TypeConv( TextAnalyse *txt, char *CSType, char *CSTypeName, bool UseSize, char *CPPType, bool *pOutFlag, bool Struct, bool Unsafe )
{
...
/*	fprintf2( cs, csW, "\t\t[StructLayout(LayoutKind.Explicit)]\n" );
	...
	fprintf2( cs, csW, "\t\t\t[FieldOffset(2)] public ushort MaterialIndex;\n" );
	fprintf2( cs, csW, "\t\t\t[FieldOffset(4)] public int VIndexTarget;\n" );
	...
	for(;;)
	{
		...
		if( strcmp( Str, "struct" ) == 0 )
		{
			...
			for(;;)
			{
				...
				// 型を C# 用に変換する
				size = TypeConv( &ttxt, Str2, Str4, true, Str3, NULL, true, false );
				for(;;)
				{
					// 変数名を取得
					ttxt.getstr( Str );
...
void OutputFunctionStr( FILE *cs, FILE *csW, FILE *sc, FILE *scW, FILE *def, FILE *defW, FILE *def64, FILE *def64W, FILE *hd, FILE *hdW )
{
	...
	for(;;)
	{
		...
		// 戻り値の型を取得する
		size = TypeConv( &ttxt, Func.CSType, Func.CSTypeName, false, Func.CPPType, NULL, false, unsafe );
...
*/
		...
		// 引数解析
		{
			...
			else
			{
				...
				for(;;)
				{
					...
					// C#用の引数の型を得る
					TypeConv( &ttxt, par->CSType, par->CSTypeName, false, par->CPPType, &par->Out, false, unsafe );
		...
		// C#用関数出力
		{
			...
			{
				// 戻り値の型と関数名の出力
				fprintf2( cs, csW, "\t\tpublic %sstatic %s %s( ", unsafe ? "unsafe " : "", Func.CSType, Func.Name );
